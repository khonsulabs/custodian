...
fn cipher_suites(criterion: &mut Criterion) {
	...
	criterion.bench_function("Ristretto225 + SHA-512 + Argon2id", |bencher| {
		bencher.iter(|| {
			cipher_suite(Ake::Ristretto255, Group::Ristretto255, Hash::Sha2, argon2id).unwrap()
		})
	});
	criterion.bench_function("Ristretto225 + SHA-512 + Argon2d", |bencher| {
		bencher.iter(|| {
			cipher_suite(Ake::Ristretto255, Group::Ristretto255, Hash::Sha2, argon2d).unwrap()
		})
	});
	#[cfg(feature = "pbkdf2")]
	...
	criterion.bench_function("Ristretto225 + SHA3-512 + Argon2id", |bencher| {
		bencher.iter(|| {
			cipher_suite(Ake::Ristretto255, Group::Ristretto255, Hash::Sha3, argon2id).unwrap()
		})
	});
	#[cfg(feature = "sha3")]
	criterion.bench_function("Ristretto225 + SHA3-512 + Argon2d", |bencher| {
		bencher.iter(|| {
			cipher_suite(Ake::Ristretto255, Group::Ristretto255, Hash::Sha3, argon2d).unwrap()
		})
	});
	...
	criterion.bench_function("P256 + SHA-256 + Argon2id", |bencher| {
		bencher.iter(|| cipher_suite(Ake::P256, Group::P256, Hash::Sha2, argon2id).unwrap())
	});
	#[cfg(feature = "p256")]
	criterion.bench_function("P256 + SHA-256 + Argon2d", |bencher| {
		bencher.iter(|| cipher_suite(Ake::P256, Group::P256, Hash::Sha2, argon2d).unwrap())
	});
	#[cfg(all(feature = "p256", feature = "pbkdf2"))]
	criterion.bench_function("P256 + SHA-256 + PBKDF2-SHA256", |bencher| {
		bencher.iter(|| cipher_suite(Ake::P256, Group::P256, Hash::Sha2, pbkdf2sha256).unwrap())
	});
	#[cfg(all(feature = "p256", feature = "pbkdf2"))]
	criterion.bench_function("P256 + SHA-256 + PBKDF2-SHA512", |bencher| {
		bencher.iter(|| cipher_suite(Ake::P256, Group::P256, Hash::Sha2, pbkdf2sha512).unwrap())
	});
	#[cfg(all(feature = "p256", feature = "sha3"))]
	criterion.bench_function("P256 + SHA3-256 + Argon2id", |bencher| {
		bencher.iter(|| cipher_suite(Ake::P256, Group::P256, Hash::Sha3, argon2id).unwrap())
	});
	#[cfg(all(feature = "p256", feature = "sha3"))]
	criterion.bench_function("P256 + SHA3-256 + Argon2d", |bencher| {
		bencher.iter(|| cipher_suite(Ake::P256, Group::P256, Hash::Sha3, argon2d).unwrap())
	});
	#[cfg(all(feature = "p256", feature = "sha3", feature = "pbkdf2"))]
	criterion.bench_function("P256 + SHA3-256 + PBKDF2-SHA256", |bencher| {
		bencher.iter(|| cipher_suite(Ake::P256, Group::P256, Hash::Sha3, pbkdf2sha256).unwrap())
	});
	#[cfg(all(feature = "p256", feature = "sha3", feature = "pbkdf2"))]
	criterion.bench_function("P256 + SHA3-256 + PBKDF2-SHA512", |bencher| {
		bencher.iter(|| cipher_suite(Ake::P256, Group::P256, Hash::Sha3, pbkdf2sha512).unwrap())
	});
	#[cfg(all(feature = "p256", feature = "blake3"))]
	criterion.bench_function("P256 + BLAKE3 + Argon2id", |bencher| {
		bencher.iter(|| cipher_suite(Ake::P256, Group::P256, Hash::Blake3, argon2id).unwrap())
	});
	#[cfg(all(feature = "p256", feature = "blake3"))]
	criterion.bench_function("P256 + BLAKE3 + Argon2d", |bencher| {
		bencher.iter(|| cipher_suite(Ake::P256, Group::P256, Hash::Blake3, argon2d).unwrap())
	});
	#[cfg(all(feature = "p256", feature = "blake3", feature = "pbkdf2"))]
	criterion.bench_function("P256 + BLAKE3 + PBKDF2-SHA256", |bencher| {
		bencher.iter(|| cipher_suite(Ake::P256, Group::P256, Hash::Blake3, pbkdf2sha256).unwrap())
	});
	#[cfg(all(feature = "p256", feature = "blake3", feature = "pbkdf2"))]
	criterion.bench_function("P256 + BLAKE3 + PBKDF2-SHA512", |bencher| {
		bencher.iter(|| cipher_suite(Ake::P256, Group::P256, Hash::Blake3, pbkdf2sha512).unwrap())
	});
}
